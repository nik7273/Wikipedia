...
filename2 = "../Harrison's/STEMI.pdf"
HarrTxt = "HarrisonTxtFromPDF2.txt"
...
with codecs.open('STEMI_words',WRITE,'utf-8') as outfile:
    for word in data2:
        print>>outfile,word
        
distri2 = nltk.FreqDist(codecs.open('STEMI_words',READ).read().splitlines())

common2 = distri2.most_common(30)
words2,freqs2 = zip(*common2)
with codecs.open("listedData3.txt",WRITE,'utf-8') as outfile:
    for x,y in common2:
...
plt.tight_layout()
plt.show()
plt.savefig("Harrisons-STEMI-word-frequencies.png", bbox_inches="tight")
